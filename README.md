...
> In this tutorial case, we are using the Push model where the publisher holds the responsibility of knowing when a change occurs and immediately broadcasting the relevant data to all registered subscribers via an HTTP POST request. The subscribers are passive in this exchange, they simply sit and wait for the publisher to push the information to their respective endpoints whenever an event like a product creation or deletion is triggered.

**2. What are the advantages and disadvantages of using the other variation of Observer Pattern for this tutorial case? (example: if you answer Q1 with Push, then imagine if we used Pull)**
> A Pull model would reduce the noise for subscribers because they would only request data when they actually need it, which can prevent them from being overwhelmed by too many updates. It also makes the publisher simpler because it doesn't need to track the state or delivery success for every single subscriber. But in the context of this tutorial, the Pull model would be much less efficient since subscribers would have to constantly pull to the publisher to check for updates, leading to a massive amount of unnecessary HTTP traffic if no changes have occurred. Furthermore, there would be a delay between the actual event and when the subscriber finally pulls the data, making the system lose its real-time feature.

**3. Explain what will happen to the program if we decide to not use multi-threading in the notification process.**
> If we decided to processed notifications synchronously, the entire program would experience a significant performance bottleneck. In a single-threaded environment, the publisher would have to send an HTTP request to the first subscriber and wait for a response before moving on to the second, and so on. If one subscriber has a slow network or a lagging server, the entire application would freeze until that specific request times out or completes. By using multi-threading, we allow the publisher to fire off all notifications simultaneously, ensuring that the main application remains responsive and no subscriber delay the notification of everyone else.
